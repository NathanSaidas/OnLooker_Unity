...
namespace Gem
{
    ...
    public class UIToggleEditor : Editor
    {
        ...
        private void DrawUILabel()
        {
            ...
            if(label != null)
            {
                ...
                if(GUI.changed)
                {
                    ...
                    {
                        boxCollider.isTrigger = true;
                        label.UpdateBounds(boxCollider,1.0f);
                    }
        ...
        private void DrawUIButton()
        {
            ...
            if(button != null)
            {
                ...
                if(GUI.changed)
                {
                    ...
                    {
                        boxCollider.isTrigger = true;
                        label.UpdateBounds(boxCollider,button.minWidth);
                        if (button.autoAdjust == true)
                        {
        ...
        {
            UITextfield textfield = inspected.GetComponentInChildren<UITextfield>();
            if (textfield != null)
            {
                bool enabled = textfield.buttonState != UIButtonState.DISABLED;
                enabled = EditorGUILayout.Toggle(UIEditor.BUTTON_STATE, enabled);
                if (enabled == true)
                {
                    textfield.Enable();
                ...
                textfield.disabledTexture = EditorUtilities.textureField(UIEditor.DISABLED, textfield.disabledTexture);
                textfield.normalTexture = EditorUtilities.textureField(UIEditor.NORMAL, textfield.normalTexture);
                textfield.hoverTexture = EditorUtilities.textureField(UIEditor.HOVER, textfield.hoverTexture);
                textfield.downTexture = EditorUtilities.textureField(UIEditor.DOWN, textfield.downTexture);
                textfield.enabledTextColor = EditorGUILayout.ColorField(UIEditor.ENABLED_TEXT_COLOR, textfield.enabledTextColor);
                textfield.disabledTextColor = EditorGUILayout.ColorField(UIEditor.DISABLED_TEXT_COLOR, textfield.disabledTextColor);
                textfield.eventListener = EditorUtilities.ObjectField<UIEventListener>(UIEditor.UI_EVENT_LISTENER, textfield.eventListener);
                textfield.maxCharacter = EditorGUILayout.IntField(UIEditor.MAX_CHARACTER, textfield.maxCharacter);
                ...
                if (GUI.changed)
                {
                    ...
                    {
                        boxCollider.isTrigger = true;
                        label.UpdateBounds(boxCollider,textfield.minWidth);
                        if (textfield.autoAdjust == true)
                        {
                            image.width = boxCollider.size.x;
                            image.height = boxCollider.size.y;
                        }
