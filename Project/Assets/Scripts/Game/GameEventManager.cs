...
namespace Gem
{
    ...
    public class GameEventManager : MonoBehaviour
    {
        ...
        private HashSet<IGameEventReceiver> m_UnitEventListener = new HashSet<IGameEventReceiver>();
        private HashSet<IGameEventReceiver> m_TriggerEventListener = new HashSet<IGameEventReceiver>();
        #endregion
        #endregion
        ...
        public static void UnregisterEventListener(GameEventType aType, IGameEventReceiver aListener)
        {
            if(aListener == null)
            {
                ...
                {
                    case GameEventType.GAME:
                        instance.m_GameEventListener.Remove(aListener);
                        break;
                    case GameEventType.UNIT:
                        instance.m_UnitEventListener.Remove(aListener);
                        break;
                    case GameEventType.TRIGGER:
                        instance.m_TriggerEventListener.Remove(aListener);
                        break;
                }
            }
        }
        ...
        private void ProcessGameEvent(ref GameEventData aEvent)
        {
            ...
            switch(aEvent.eventType)
            {
                ...
                case GameEventType.UNIT:
                    {
                        IEnumerator<IGameEventReceiver> receivers = m_UnitEventListener.GetEnumerator();
                        while(receivers.MoveNext())
                        {
                            if(receivers.Current == null)
                            {
                ...
                case GameEventType.TRIGGER:
                    {
                        IEnumerator<IGameEventReceiver> receivers = m_TriggerEventListener.GetEnumerator();
                        while(receivers.MoveNext())
                        {
                            if(receivers.Current == null)
                            {
